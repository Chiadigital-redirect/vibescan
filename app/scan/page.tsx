...
function ScanPageInner() {
  ...
  function handleExport() {
    ...
    const date = new Date(report.scannedAt).toLocaleString();
    const scoreLabel =
      report.score >= 70 ? "✅ Good" : report.score >= 40 ? "⚠️ Needs Work" : "🔴 Critical";

    const criticalLines = report.checks
      .filter(c => c.status === 'critical')
      .map(c => `- **${c.name}**: ${c.detail}`)
      .join('\n');

    const warningLines = report.checks
      .filter(c => c.status === 'warning')
      .map(c => `- **${c.name}**: ${c.detail}`)
      .join('\n');

    const passLines = report.checks
      .filter(c => c.status === 'pass')
      .map(c => `- ${c.name}`)
      .join('\n');

    const dbSection = report.dataLeaks && report.dataLeaks.tables.length > 0
      ? `## 🗄️ Database Exposure\n\n` +
        `Supabase URL: \`${report.dataLeaks.supabaseUrl}\`\n` +
        `Anon Key (preview): \`${report.dataLeaks.keyPreview}\`\n\n` +
        `| Table | Row Count | RLS Enabled |\n|---|---|---|\n` +
        report.dataLeaks.tables.map(t =>
          `| ${t.name} | ${t.totalRows ?? t.sampleRows.length} | ${t.rls ? '✅' : '❌ None'} |`
        ).join('\n')
      : '';

    const urlList = report.discoveredUrls.slice(0, 50).map(u => `- ${u}`).join('\n');
    const moreUrls = report.discoveredUrls.length > 50
      ? `\n_… and ${report.discoveredUrls.length - 50} more_`
      : '';

    const md = `# VibeScan Security Report

**URL:** ${report.url}  
**Scanned:** ${date}  
**Score:** ${report.score}/100 ${scoreLabel}
...
${criticalLines ? `## 🔴 Critical Issues — Fix Immediately\n\n${criticalLines}\n\n---\n` : ''}
${warningLines ? `## 🟡 Warnings — Fix Soon\n\n${warningLines}\n\n---\n` : ''}
${passLines ? `## ✅ Passed Checks\n\n${passLines}\n\n---\n` : ''}
${dbSection ? `${dbSection}\n\n---\n` : ''}
## 🗺️ Pages Discovered (${report.discoveredUrls.length})

${urlList}${moreUrls}

---

_Report generated by [VibeScan](https://vibescan.io) — built by [Creative Digital Group](https://creativedigital.group)_  
_Passive scan only. Results reflect publicly accessible information._
`;
    ...
    try {
      ...
      a.href = url;
      a.download = `vibescan-${domain}-${new Date().toISOString().slice(0, 10)}.md`;
        ...
        <div className="max-w-5xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="flex items-center gap-1.5 text-sm px-3 py-2 border border-slate-200 rounded-lg hover:bg-slate-50 transition-colors text-slate-600 font-medium"
            >
